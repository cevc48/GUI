...
class IntegratedUAVGUI:
    def __init__(self, root):
        ...
        main_frame.pack(fill="x", expand=True, anchor="center")

        left_frame = ttk.Frame(main_frame,width=200)
        left_frame.pack(side="left", fill="x", expand='True',anchor='w')

        right_frame = ttk.Frame(main_frame, width=100)
        right_frame.pack(side="right", padx=10,anchor='e',fill='y')

        lower_frame = ttk.Frame(main_frame,width=30)
        lower_frame.pack(side='bottom',anchor='e',fill='x',padx=5)
        ...
        self.canvas = tk.Canvas(instrument_tab, width=400, height=400, relief='groove', borderwidth=3,
        ...
        self.compass_needle = self.canvas.create_line(200, 200, 200, 80, width=3, fill='red', arrow='last', smooth=True)
        self.draw_heading_marks()

        self.horizon_canvas = tk.Canvas(instrument_tab, width=400, height=300, bg="black", relief='groove', border="2",
                                        borderwidth=1)
        self.horizon_canvas.pack(pady=10, anchor="center")
        self.sky = self.horizon_canvas.create_rectangle(0, 0, 400, 150, fill="skyblue", width=1)
        self.ground = self.horizon_canvas.create_rectangle(0, 150, 400, 300, fill="saddlebrown", width=1)
        ...
        self.map_widget = TkinterMapView(map_tab, width=600, height=300, corner_radius=1)
        ...
        # Vertical Gauges
        self.alt_gauge = tk.Canvas(right_frame, width=50, height=220, bg='black')
        self.alt_gauge.pack(pady=5,padx=5)
        self.alt_label = ttk.Label(right_frame, text="ALT:\n0 m", font=('Arial', 10), background='yellow',
                                   anchor="center", width='10')
        self.alt_label.pack()

        self.speed_gauge = tk.Canvas(right_frame, width=50, height=220, bg='black')
        self.speed_gauge.pack(pady=5,padx=5)
        self.speed_label = ttk.Label(right_frame, text="VEL:\n0 km/h", font=('Arial', 10), background='lightgreen',
                                     anchor="center", width='10')
        self.speed_label.pack()

        self.battery_label = ttk.Label(lower_frame, text="BAT:\n0.0 V", font=('Arial', 10), background='lightblue',
                                       anchor="center", width='10')
        self.battery_label.pack(pady=(20, 10))

        self.battery_alert = ttk.Label(lower_frame, text="", font=('Arial', 10), foreground='red', width='10',anchor='n')
        self.battery_alert.pack(pady=5)
        self.flightmode_label = ttk.Label(lower_frame, text="Modo: ---", font=('Arial', 10), background='white',
                                          anchor="n", width='10')
        self.flightmode_label.pack()
        self.rssi_label = ttk.Label(lower_frame, text="RSSI: ---%", font=('Arial', 10), background='white',
                                    anchor="n", width='10')
        ...
        self.sim_toggle.pack(pady=5)
        # Waypoint buttons
        self.wp_create_button = ttk.Button(lower_frame, text="Criar Waypoint", command=self.add_current_waypoint,
                                           width='20')
        self.wp_create_button.pack(anchor='ne')

        self.wp_send_button = ttk.Button(lower_frame, text="Enviar Waypoints", command=self.send_waypoints, width='20')
        self.wp_send_button.pack(anchor='ne')
        self.wp_save_button = ttk.Button(lower_frame, text="Gravar Waypoints", command=self.save_waypoints, width='20')
        self.wp_save_button.pack(anchor='ne')

        self.wp_load_button = ttk.Button(lower_frame, text="Carregar Waypoints", command=self.load_waypoints, width='20')
        self.wp_load_button.pack(anchor='ne')
        self.rth_button = ttk.Button(lower_frame, text="Gerar RTH", command=self.generate_rth_path, width='20')
        self.rth_button.pack(anchor='ne')
        self.edit_wp_button = ttk.Button(lower_frame, text="Editar Último WP", command=self.edit_last_waypoint,
                                         width='20')
        self.edit_wp_button.pack(anchor='ne')

        self.del_wp_button = ttk.Button(lower_frame, text="Apagar Último WP", command=self.delete_last_waypoint,
                                        width='20')
        self.del_wp_button.pack(anchor='ne')
    ...
    def simulation_loop(self):
        ...
        while self.running:
            ...
            mode = random.choice(['MANUAL', 'LOITER', 'NAVIGATION', 'RTH'])
